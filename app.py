...
def run_threads_scraper(api_key:str, usernames:List[str], max_posts:int=50) -> List[Dict[str,Any]]:
    ...
    run_input={"urls":[f"https://www.threads.net/@{u}" for u in usernames],
               "maxPosts": max_posts, "includeComments": False}
...
def process_profiles_and_posts(data: List[Dict[str,Any]]):
    ...
    for uname,p in profiles.items():
        ...
        rationale=[f"Bio sentiment scaled: {round(bio_score*10,1)}",
                   f"Relevant posts(capped5): {min(rel,5)}",
                   f"Avg engagement scaled(max3): {round(min(avg_eng/50,3),2)}"]
        tags_bio=p.get("Keyword Hits Bio",{})
        extra=[]
        if tags_bio.get("onco"): extra.append("Oncology")
        if tags_bio.get("gi"): extra.append("GI")
        if tags_bio.get("res"): extra.append("Research")
        if tags_bio.get("brand"): extra.append("Brand/Drugs")
        if extra: rationale+=["Bio mentions: "+", ".join(extra)]
        p["DOL Score"]=score
